"""Full project scan for untranslated strings - v3 (fixed whitespace)"""
...
def add(s, fp, t):
    if not s or len(s) <= 2:
        return
    # Check if s is in msgids
    if s in msgids:
        return
    # Also check if any msgid starts with s (for truncated strings)
    if len(s) > 10:
        for mid in msgids:
            if mid.startswith(s):
                return
...
for fp in html_files:
    try:
        ...
        continue
    # {% trans %}...{% endtrans %} - keep exact whitespace
    for m in re.finditer(r'\{%\s*trans\s*%\}\s*(.+?)\s*\{%\s*endtrans\s*%\}', c, re.DOTALL):
        s = m.group(1).strip()
        # Don't normalize whitespace - compare as-is
        # But also check normalized version
        s_norm = re.sub(r'\s+', ' ', s)
        if s not in msgids and s_norm not in msgids:
