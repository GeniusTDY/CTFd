...
def test_with_margin(margin_top):
    """测试指定 margin-top 下的留白"""
    # 先修改 margin-top
    import re as re2
    html_path = '/workspace/CTFd/themes/admin/templates/statistics.html'
    with open(html_path, 'r') as f:
        content = f.read()
    content = re2.sub(
        r'#categories-pie-graph \{\s*margin-top: \d+px !important;',
        f'#categories-pie-graph {{\n        margin-top: {margin_top}px !important;',
        content
    )
    ...
    with sync_playwright() as p:
        ...
        m_page.screenshot(path=f'/workspace/_test_m{margin_top}.png', full_page=True)
        m_context.close()
        browser.close()

    img = Image.open(f'/workspace/_test_m{margin_top}.png')
    ...
    # 2. Legend 上留白（用彩色像素识别环形图底部，排除 label 干扰）
    legend_gaps = {}
    ...
    return gap, legend_gaps, opt_info

# 测试 margin-top=108（原始）
print("=" * 60)
print("测试 margin-top=108（原始值）")
print("=" * 60)
gap108, leg108, opt108 = test_with_margin(108)
print(f"  正确/错误提交 → 分类细分标题 留白: {gap108:.1f}px")
print(f"  ECharts option: cat.legend.top={opt108['#categories-pie-graph']['legend_top']}, pt.legend.top={opt108['#points-pie-graph']['legend_top']}")
for label, g in leg108.items():
    print(f"  {label} Legend 上留白(彩色像素): {g:.1f}px")

# 测试 margin-top=98（减少10px的目标值）
print("\n" + "=" * 60)
print("测试 margin-top=98（目标值）")
print("=" * 60)
gap98, leg98, opt98 = test_with_margin(98)
print(f"  正确/错误提交 → 分类细分标题 留白: {gap98:.1f}px")
print(f"  ECharts option: cat.legend.top={opt98['#categories-pie-graph']['legend_top']}, pt.legend.top={opt98['#points-pie-graph']['legend_top']}")
for label, g in leg98.items():
    print(f"  {label} Legend 上留白(彩色像素): {g:.1f}px")

print("\n" + "=" * 60)
print("对比")
print("=" * 60)
print(f"  留白变化: {gap108:.1f} → {gap98:.1f} = 减少 {gap108 - gap98:.1f}px")
print(f"  分类细分 Legend 留白: {leg108.get('分类细分', 0):.1f} → {leg98.get('分类细分', 0):.1f}")
print(f"  积分细分 Legend 留白: {leg108.get('积分细分', 0):.1f} → {leg98.get('积分细分', 0):.1f}")
