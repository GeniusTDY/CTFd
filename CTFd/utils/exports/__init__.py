...
def import_ctf(backup, erase=True, ignore_overrides=False):
    ...
    if alembic_version in (
        ...
        too_old_error = gettext(
            "The version of CTFd that this backup is from is too old to be automatically imported."
        )
        set_import_error(gettext("Exception:") + " " + too_old_error)
        raise Exception(too_old_error)
    ...
    if sqlite is False and alembic_version not in get_available_revisions():
        migration_error = gettext(
            "The target migration in this backup is not available in this version of CTFd."
        )
        set_import_error(gettext("Exception:") + " " + migration_error)
        raise Exception(migration_error)
            ...
            if member.startswith("db/"):
                ...
                if data:
                    ...
                    for i, entry in enumerate(saved["results"]):
                        set_import_status(
                            gettext("inserting %(member)s %(i)d/%(count)d")
                            % {"member": member, "i": i, "count": count}
                        )
                    ...
                    if postgres:
                        ...
                        if '"' not in table_name and "'" not in table_name:
                            ...
                            quotes_error = gettext(
                                "Table name %(table_name)s contains quotes"
                            ) % {"table_name": table_name}
                            set_import_error(gettext("Exception:") + " " + quotes_error)
                            raise Exception(quotes_error)
    ...
    for f in files:
        ...
        if os.path.isabs(filename) or ".." in filename:
            invalid_upload_error = gettext("Encountered invalid upload file in import")
            set_import_error(invalid_upload_error)
            raise Exception(invalid_upload_error)
