...
def import_ctf(backup, erase=True, ignore_overrides=False):
    ...
    try:
        ...
        set_import_error(
            gettext("Exception: Could not determine appropriate database version")
        )
        raise Exception(
            gettext(
                ...
                if data:
                    ...
                    if postgres:
                        ...
                        if '"' not in table_name and "'" not in table_name:
                            ...
                            set_import_error(
                                gettext(
                                    "Exception: Table name %(table_name)s contains quotes"
                                )
                                % {"table_name": table_name}
                            )
                            raise Exception(
    ...
    for plugin in plugins:
        set_import_status(
            gettext("inserting plugin %(plugin)s") % {"plugin": plugin}
        )
    ...
    for f in files:
        ...
        if os.path.isabs(filename) or ".." in filename:
            set_import_error(
                gettext("Encountered invalid upload file in import")
            )
            raise Exception(
                gettext("Encountered invalid upload file in import")
            )
