...
Alpine.data("Challenge", () => ({
  ...
  async showSolves() {
    ...
    const _wmcFmt = _wmcLocale.startsWith('zh') ? "YYYY年MM月DD日 HH:mm:ss" : "MMMM Do, h:mm:ss A";
    this.solves.forEach(solve => {
      solve.date = dayjs(solve.date).format(_wmcFmt);
      return solve;
    });
    new Tab(this.$el).show();
  },
  ...
  async showSubmissions() {
    ...
    const _wmcLocale2 = (document.cookie.match(/language=([^;]+)/) || [])[1] || '';
    const _wmcFmt2 = _wmcLocale2.startsWith('zh') ? "YYYY年MM月DD日 HH:mm:ss" : "MMMM Do, h:mm:ss A";
    this.submissions.forEach(s => {
      s.date = dayjs(s.date).format(_wmcFmt2);
      return s;
    });
    new Tab(this.$el).show();
  },
  ...
  async submitRating() {
    ...
    if (response.value) {
      ...
      alert(window.wmcTr ? window.wmcTr("Error submitting rating") : "Error submitting rating");
    }
  },
}));
