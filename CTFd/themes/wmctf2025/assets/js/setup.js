...
Alpine.data("SetupForm", () => ({
    ...
    window.addEventListener("storage", function (event) {
      if (event.key == "integrations" && event.newValue) {
        ...
        if (integration["name"] == "mlc") {
          $("#integration-mlc").text(window.wmcTr ? window.wmcTr("Already Configured") : "Already Configured").attr("disabled", true);
          window.focus();
  ...
  validateFileSize(e, limit) {
    if (e.target.files[0].size > limit) {
      if (
        !confirm(
          `${window.wmcTr ? window.wmcTr("This image file is larger than") : "This image file is larger than"} ${
            limit / 1000
          }KB ${window.wmcTr ? window.wmcTr("which may result in increased load times. Are you sure you'd like to use this file?") : "which may result in increased load times. Are you sure you'd like to use this file?"}`,
        )
      ) {
        e.target.value = "";
      }
