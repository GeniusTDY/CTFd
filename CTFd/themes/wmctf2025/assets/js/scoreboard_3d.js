...
class Scoreboard3D {
  ...
  addChallengeLabels(group, challenge, color) {
    ...
    context.fillText(`${challenge.value} ${window.wmcTr ? window.wmcTr('pts') : 'pts'}`, canvas.width / 2, 45);

    // Solves count
    context.fillStyle = '#00FF00';
    context.font = '12px Arial';
    context.fillText(`${challenge.solves || 0} ${window.wmcTr ? window.wmcTr('solves') : 'solves'}`, canvas.width / 2, 65);
  ...
  showChallengeInfo(challenge) {
    ...
    if (nameElement) nameElement.textContent = challenge.name || (window.wmcTr ? window.wmcTr('Unknown Challenge') : 'Unknown Challenge');
    if (categoryElement) categoryElement.textContent = challenge.category || (window.wmcTr ? window.wmcTr('Unknown Category') : 'Unknown Category');
    ...
    if (descriptionElement) descriptionElement.textContent = challenge.description || (window.wmcTr ? window.wmcTr('No description available') : 'No description available');
  ...
  getDifficultyText(points) {
        if (points <= 100) return window.wmcTr ? window.wmcTr('Easy') : 'Easy';
        if (points <= 300) return window.wmcTr ? window.wmcTr('Medium') : 'Medium';
        if (points <= 500) return window.wmcTr ? window.wmcTr('Hard') : 'Hard';
        return window.wmcTr ? window.wmcTr('Expert') : 'Expert';
    }
  ...
  showTeamMovement(team, challenge) {
    ...
    if (teamActionElement) teamActionElement.textContent = `${window.wmcTr ? window.wmcTr('Solving:') : 'Solving:'} ${challenge.name}`;
        ...
        if (activityFeedElement && activityFeedElement.__x) {
          ...
          if (activityFeed && activityFeed.addActivity && team && team.name && challenge && challenge.name) {
            activityFeed.addActivity({
              ...
              type: 'solve',
              team: team.name,
              message: `${window.wmcTr ? window.wmcTr('Solved') : 'Solved'} ${challenge.name} (+${challenge.value || 0} ${window.wmcTr ? window.wmcTr('points') : 'points'})`,
              time: new Date().toLocaleTimeString(((document.cookie.match(/language=([^;]+)/) || [])[1] || '').startsWith('zh') ? 'zh-CN' : 'en-US')
            });
      }
    }
  } catch (error) {
