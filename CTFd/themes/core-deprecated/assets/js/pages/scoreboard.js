...
const buildGraphData = () => {
  return CTFd.api.get_scoreboard_detail({ count: 10 }).then(response => {
    ...
    const option = {
      title: {
        left: "center",
        text: _("Top 10 ") + (CTFd.config.userMode === "teams" ? _("Teams") : _("Users"))
      },
      tooltip: {
        trigger: "axis",
        axisPointer: {
...
const createGraph = () => {
  buildGraphData().then(option => {
    if (option === false) {
      // Replace spinner
      graph.html(
        '<h3 class="opacity-50 text-center w-100 justify-content-center align-self-center">' + _("No solves yet") + '</h3>'
      );
      return;
    }
