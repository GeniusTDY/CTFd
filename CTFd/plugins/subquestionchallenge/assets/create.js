CTFd.plugin.run((_CTFd) => {
    ...
    // Translations are injected server-side via Flask-Babel in create.html
    // as window.SubQuestionI18n. Fallback to the original English string.
    const __ = (str) => (window.SubQuestionI18n && window.SubQuestionI18n[str]) || str;

    // Translate a key with optional parameters (e.g. "Question %(num)s")
    ...
    // Translations are already available in window.SubQuestionI18n (rendered by Flask-Babel in create.html)
    initCreateForm();
});
