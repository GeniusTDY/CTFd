...
class ResetInstanceForm(BaseForm):
    accounts = BooleanField(
        _l("Accounts"),
        description=_l("Deletes all user and team accounts and their associated information"),
    )
    submissions = BooleanField(
        _l("Submissions"),
        description=_l("Deletes all records that accounts gained points or took an action"),
    )
...
class AccountSettingsForm(BaseForm):
    domain_whitelist = StringField(
        ...
        description=_l("Comma-seperated list of allowable email domains which users can register under (e.g. examplectf.com, example.com, *.example.com)"),
    )
    domain_blacklist = StringField(
        _l("Email Domain Blocklist"),
        description=_l("Comma-seperated list of disallowed email domains which users cannot register under (e.g. examplectf.com, example.com, *.example.com)"),
    )
    team_creation = SelectField(
        _l("Team Creation"),
        description=_l("Control whether users can create their own teams (Teams mode only)"),
    ...
    num_teams = IntegerField(
        ...
        widget=NumberInput(min=0),
        description=_l("Maximum number of teams allowed to register with this CTF (Teams mode only)"),
    )
    num_users = IntegerField(
        _l("Maximum Number of Users"),
        widget=NumberInput(min=0),
        description=_l("Maximum number of user accounts allowed to register with this CTF"),
    )
    ...
    verify_emails = SelectField(
        _l("Verify Emails"),
        description=_l("Control whether users must confirm their email addresses before playing"),
        choices=[("true", _l("Enabled")), ("false", _l("Disabled"))],
        default="false",
    )
    team_disbanding = SelectField(
        _l("Team Disbanding"),
        description=_l("Control whether team captains are allowed to disband their own teams"),
        choices=[
    ...
    incorrect_submissions_per_min = IntegerField(
        ...
        description=_l("Number of submissions allowed per minute for flag bruteforce protection (default: 10)"),
    )
...
class SocialSettingsForm(BaseForm):
    social_shares = SelectField(
        _l("Social Shares"),
        description=_l("Enable or Disable social sharing links for challenge solves"),
        choices=[("true", _l("Enabled")), ("false", _l("Disabled"))],
        default="true",
    )
...
class LegalSettingsForm(BaseForm):
    tos_url = URLField(
        ...
        description=_l("External URL to a Terms of Service document hosted elsewhere"),
    )
    ...
    privacy_url = URLField(
        ...
        description=_l("External URL to a Privacy Policy document hosted elsewhere"),
    )
...
class ChallengeSettingsForm(BaseForm):
    ...
    max_attempts_timeout = IntegerField(
        ...
        description=_l("How long the timeout lasts in seconds for max attempts (if set to timeout). Default is 300 seconds"),
        default=300,
    )
    hints_free_public_access = SelectField(
        _l("Hints Free Public Access"),
        description=_l("Control whether users must be logged in to see free hints (hints without cost or requirements)"),
        choices=[("true", _l("Enabled")), ("false", _l("Disabled"))],
        default="false",
    )
    challenge_ratings = SelectField(
        ...
        choices=[
            ("public", _l("Public (users can submit ratings and see aggregated ratings)")),
            (
                "private",
                _l("Private (users can submit ratings but cannot see aggregated ratings)"),
            ),
            (
                "disabled",
                _l("Disabled (users cannot submit ratings or see aggregated ratings)"),
            ),
        ],
        default="public",
    )
...
class LocalizationForm(BaseForm):
    default_locale = SelectField(
        _l("Default Language"),
        description=_l("Language to use if a user does not specify a language in their account settings. By default, CTFd will auto-detect the user's preferred language."),
...
class EmailSettingsForm(BaseForm):
    ...
    mail_username = StringField(_l("Username"), description=_l("Mail Server Account Username"))
    mail_password = StringField(_l("Password"), description=_l("Mail Server Account Password"))
    ...
    mailgun_api_key = StringField(_l("Mailgun API Key"), description=_l("Mailgun API Key"))
    ...
    successful_registration_email_body = TextAreaField(
        _l("Body"),
        description=_l("Email body sent to users after they've finished registering"),
    ...
    verification_email_body = TextAreaField(
        _l("Body"),
        description=_l("Email body sent to users to confirm their account at the address they provided during registration"),
    ...
    user_creation_email_body = TextAreaField(
        _l("Body"),
        description=_l("Email body sent to new users (manually created by an admin) with their initial account details"),
    ...
    password_change_alert_body = TextAreaField(
        _l("Body"),
        description=_l("Email body sent when a user successfully resets their password"),
