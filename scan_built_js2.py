...
result = subprocess.check_output(
    ['find', '/workspace/CTFd/themes/core-deprecated/static', '-name', '*.js', '-type', 'f'],
    text=True
).strip()
js_files = [f for f in result.split('\n') if f and 'echarts' not in f and 'vendor' not in f]
...
print(f'Missing from core-deprecated built JS: {len(missing)}')
